...
export const getServerSideProps: GetServerSideProps = async ({ query, req }) => {
  ...
  if (data) {
    ...
    try {
      const dataFetched = await fetch("http://localhost:3000/api/saveuser", {
        method: "POST",
        mode: "cors",
        body: JSON.stringify({ name: data.name, picture: data.avatar_url }),
      });

      console.log(await dataFetched.json());
    } catch (err) {
